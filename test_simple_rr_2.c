...
//+ Count the corresponding quantum according to priority
int count_qq(int prio)
{
	int qq;

	//+ The quantum will be exponentially decrease
	if( !(qq = HZ / (prio + 1)) )
		qq = 1;	
	
	//+ The quantum will be: 1, 2, 1, 2 ...
	//qq = (i % 2) + 1;	
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < num_threads; i++)
	{
		...
		syscall (SYS_simple_rr_setquantum, count_qq(i));
